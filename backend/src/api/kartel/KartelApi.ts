...
import { isAdmin } from "../middleswares/PermsMiddleware";
...
export const KartelApi = (pointerApp: Application) => {
  ...
  app.post("/createPlayer", isAdmin, (req: Request, res: Response) => {
    console.log("here");
  ...
  app.post("/start", (req: Request, res: Response) => {
    ...
    if (lobby) {
      if (lobby.players[0].playerId === req.body.playerId) {
        ...
        res.status(400).send("Le player qui émit la requête n'est pas le bon");
      }
    } else {
      res.sendStatus(400);
    }
